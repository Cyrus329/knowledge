...
function renderDetail(){
  ...
  const activePanel = i.pageCloze ? 'cloze' : 'learn';
  const learnHTML = i.pageCloze
  ...
  pane.innerHTML=`<button class="back" id="backBtn">← 返回列表</button><div class="dhead slim-head"><div><p>${esc(i.subject)} · ${esc(i.chapter)} · ${esc(i.range||'')}</p><h2>${esc(i.title)}</h2></div><span>${String(i.order||'').padStart(3,'0')}</span></div><div class="ops slim-ops"><button data-act="read" class="${s.read?'on':''}">${s.read?'已看':'标已看'}</button><button data-act="star" class="star ${s.starred?'on':''}">重点</button><button data-act="master" class="master ${s.mastered?'on':''}">${s.mastered?'已掌握':'掌握'}</button><button data-act="forget" class="memory-on">抗遗忘</button></div><div class="tabs slim-tabs">${tab('learn','背诵整理')}${tab('cloze','填空背诵')}${tab('pdf','PDF原图/干净整理')}${tab('exam','考试考法')}${tab('confuse','易混/图表')}${tab('phrase','例句/短语')}${tab('test','自测')}${tab('memory','抗遗忘')}</div>${panel('learn',learnHTML)}${panel('cloze',clozeHTML(i))}${panel('pdf',`${sec('原图 / 答案页', mediaHTML(i), true)}${!i.pageCloze ? sec('PDF内容提取：清爽整理版', cleanExtractHTML(i), true) : ''}`)}${panel('exam',`${sec('考点提炼', list(i.examRefine||[]), true)}${sec('必背句式 / 固定结构', list(i.mustPatterns||[]), true)}`)}${panel('confuse',`${confusionHTML(i.confusions||[])}${(i.tables||[]).map(tableHTML).join('')}`)}${panel('phrase',`${examplesHTML(i.examples||[])}${phraseHTML(i.phraseGroups||[])}`)}${panel('test',`${testsHTML(i.selfTests||[])}<div class="note"><h3>我的笔记</h3><textarea data-note="${esc(i.id)}" placeholder="写自己的理解、易错点...">${esc(s.note||'')}</textarea></div>`)}${panel('memory',memoryHTML(i))}`;
}
